...
def build_initial_x(rng, d):
    ...
    for m in range(n_orders):
        ...
        n_items = len(ims)
        is_opened = d.orders[m].order_id in d.opened_order_ids
    ...
    for i, budget in enumerate(budgets):
        ...
        if model.SolCount == 0:
            if i == 0:
                # Nessuna soluzione nel primo budget: rilassa EC14 e riprova
                logging.warning(
                    "[build_initial_x] No solution in first budget, relaxing EC14."
                )
                ...
                if model.SolCount == 0:
                    model.computeIIS()
                    model.write("iis.ilp")
                    model.dispose()  # rilascia risorse Gurobi
                    raise RuntimeError(
                        "[build_initial_x] No feasible solution found. IIS in iis.ilp"
                    )
            # Iterazione successiva senza soluzione: usa quella precedente
            solved = True
            break
    ...
    model.dispose()  # rilascia sempre le risorse Gurobi alla fine
    return x_sol