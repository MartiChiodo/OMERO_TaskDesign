...
# config
PODS_INIT     = 5       # pods freed per destroy at the start
PODS_MAX      = 20      # max pods freed
...
def repair(d, x_inc, y_inc, f_inc, g_inc, v_inc, freed_items, freed_pods, affected,
           total_active, total_arr, ws_loc_to_w, precomp,
    ...
    for m in affected:
        ...
        for t in range(1, T):
            ...
            mdl.addConstr(g[m, t] >= gp.quicksum(xv(im, t-1) for im in ims) - (n_it - 1))  # EC22
        for im in ims:                                         # EC16 EC18 EC19
            for t in range(T):
                mdl.addConstr(f[m, t] >= xv(im, t))
            for t in range(1, T):
                mdl.addConstr(xv(im, t) - xv(im, t-1) <= v[m, t])
                mdl.addConstr(g[m, t] <= xv(im, t-1))
        if d.orders[m].order_id in d.opened_order_ids:         # EC23
            mdl.addConstr(v[m, 0] == 1.0)
        else:
            for t in range(T):
                mdl.addConstr(f[m, t] <= gp.quicksum(xv(im, t) for im in ims))
    ...
    for w, order_ids in enumerate(d.orders_by_workstation):
        ...
        if not any(m in affected for m in ids):
            continue
        for t in range(T):
            mdl.addConstr(gp.quicksum(vv(m, t) for m in ids) <= d.OptManager.CAP_WS)
    ...
    for im in freed_items:
        ...
        for t in range(T):
            ...
            dx = xv(im, t) if t == 0 else (xv(im, t) - xv(im, t-1))
            mdl.addConstr(gp.quicksum(y[(p, a)] for a in inc) >= dx)

    # loads from fixed pods, total minus freed
    ...
    for w, order_ids in enumerate(d.orders_by_workstation):
        ...
        for t in range(1, T):
            item_work = d.OptManager.DELTA_ITEM * gp.quicksum(
                xv(im, t) - xv(im, t-1) for im in ims_w)
...
def lns_stage2(d, time_budget=TIME_BUDGET, seed=SEED):
    ...
    while time.perf_counter() - t0 < time_budget:
        ...
        since_check += 1
        stall = 0
        logging.info("[lns_stage2] iter %d | obj = %.4f | delta = + %.4f "
