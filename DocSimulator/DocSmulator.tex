...
\texttt{scripts/opt/opt\_solver.py}   & Definition of the optimization solver \\
\texttt{scripts/opt/local\_search.py} & Definition of the local search algorithm used by the otimization solver\\
...
Constraints \eqref{M1_Constr4} and \eqref{M1_Constr5} are needed to enforce balance between workstation worloads; the parameters $\overline{I}$ and $\underline{I}$ can be tuned to control the maximum allowed imbalance, or even removed to allow any feasible solution. At the moment,  $\overline{I} = \frac{num\_orders}{4} * 1.5$ and $\overline{I} = \frac{num\_orders}{4} * 0.5$.

This optimization problem is solved using Gurobi.
...
\begin{align}
    \max \quad
    & \sum_{t \in \mathcal{T}} \sum_{m \in \mathcal{M}} g_{mt} 
    + 0.5 * \sum_{i \in \mathcal{I}} \sum_{m \in \mathcal{M}} x_{imT}
    - 0.5 * \sum_{m \in \mathcal{M}} \beta_m(1 - g_{mT})
    \label{M2_Obj} \\[4pt]
    \text{s.t.} \quad
...
The objective function \eqref{M2_Obj} consists of 3 terms: the first one minimize the completion time of all orders (since $g$ is a cumulative variable, the earlier an order is completed the more terms $g_{mt}$ will be equal to $1$); the second one maximize the number of picked items at the end of the time horizon (this term is needed to encourage picking items for an order even if it won't be closed by the end); the third one penalizes uncompleted orders at the end of the time horizon, with a weight $\beta_m$ proportional to the difference between the order's arrival time and its completion time (this term is needed to ensure that orders won't be left in the backlog). \\
