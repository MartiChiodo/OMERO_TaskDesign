...
\begin{itemize}
    ...
    \item \texttt{TimeWeightedMeanTracker}: this type of statistic is used to compute time-averaged KPIs, such as the average number of open orders per workstation and the average number of pods simultaneously out of their storage location.
\end{itemize}
...
The decision variables of the first stage are:
\begin{itemize}
    \item $z_{mw} = 1$ if order $m$ is assigned to workstation $w$, $0$ otherwise.
    \item $x_{imp} = 1$ if item $i$ of order $m$ is assigned to pod $p$, $0$ otherwise.
    \item $y_{wp} = 1$ if pod $p$ is assigned to workstation $w$, $0$ otherwise.
\end{itemize}

In the code implementation, the variable $x_{imp}$ has only 2 dimensions to reduce the number of unused variables as not all items $i$ are required by order $m$. A bijective map $(i, m) \rightarrow im$ is been used to keep track of the correspondence between the two representations.

\begin{align}
    \min \quad 
    &  \sum_{w \in \mathcal{W}}\sum_{y \in \mathcal{P}} y_{wp}\\[4pt]
    \text{s.t.} \quad
...
Constraints \eqref{M1_Constr4} and \eqref{M1_Constr5} are needed to enforce balance between workstation worloads; the parameters $\overline{I}$ and $\underline{I}$ can be tuned to control the maximum allowed imbalance between workstations workload and are set at $\overline{I} = \frac{\sum_{m \in \mathcal{M}} |\mathcal{I}_m |}{4} * 1.25$ and $\overline{I} = \frac{\sum_{m \in \mathcal{M}} |\mathcal{I}_m |}{4} * 0.75$.

This optimization problem is solved using a local search strategy.

\subsubsection{Second stage: Task Design and Scheduling}
The decision variables of the second stage are:
\begin{itemize}
    \item $x_{imt} = 1$ if item $i$ of order $m$ is picked by time $t$, $0$ otherwise.
    \item $f_{mt} = 1$ if at least $1$ item of order $m$ has been picked by time $t-1$, $0$ otherwise.
    \item $g_{mt} = 1$ if order $m$ is completed by time $t-1$, $0$ otherwise.
    \item $v_{mt} = 1$ if order $m$ is open at time $t$, $0$ otherwise.
    \item $y_{pa} = 1$ if arc $a$ in the time-space network is selected for pod $p$, $0$ otherwise.
\end{itemize}

As before, the variable $x_{imt}$ has only 2 dimensions to reduce the number of unused variables as not all items $i$ are required by order $m$. A bijective map $(i, m) \rightarrow im$ is been used to keep track of the correspondence between the two representations.
Moreover, the variable $y_{pa}$ is defined only for the pods $p$ that have been assigned to a workstation in the first stage, to reduce the number of unused variables.

\begin{align}
    \max \quad
    & \sum_{i \in \mathcal{I}} \sum_{m \in \mathcal{M}} x_{im, T} - \sum_{m \in \mathcal{M}} \sum_{t \in \mathcal{T}} \beta_m(t) (1 - g_{mt}) \label{M2_Obj} \\[4pt]
    \text{s.t.} \quad
    ...
    - \sum_{a \in \mathcal{A}^+_{n}} y_{pa} = 0
     \qquad \forall p \in \mathcal{P},\; n \in \mathcal{N} \setminus \left(\mathcal{N}_{\text{end}} \cup \{N_p^0\}\right)
    \label{M2_Constr4} \\
    & x_{imt} - x_{im,t-1}
    ...
    & g_{mt} \le x_{imt}
     \qquad \forall m \in \mathcal{M},\, i \in \mathcal{I}_m,\, t \in \mathcal{T} \\
    & g_{m,t+1} \ge \sum_{i \in \mathcal{I}_m} x_{imt} - \left(|\mathcal{I}_m| - 1\right)
    ...
    & g_{mt} \ge g_{m,t-1}
     \qquad \forall m \in \mathcal{M},\, t \in \mathcal{T} \\
    & \sum_{p \in \mathcal{P}} \sum_{a \in \mathcal{A}^{\text{mov}}_p(t)} y_{pa} \leq |\mathcal{R}|
\qquad \forall\, t \in \mathcal{T} \label{max_active_pods}
\end{align}

The objective function \eqref{M2_Obj} consists of 2 terms: the first one maximize the throughput; the second one penalizes uncompleted orders at the end of the time horizon, with a weight $\beta_m$ proportional to the difference between the order's arrival time and its completion time (this term is needed to ensure that orders won't be left in the backlog). \\
Constraints \eqref{IC} and \eqref{Enforcing_IC} are needed for enforcing the initial conditions given by the orders already opened at workstations, for these orders the variable $f_{mt}$ loses its meaning as it is set to $1$ before any items is picked. \\
Constraint \eqref{max_active_pods} bounds the maximum number of pods out of their storage location simultaneously. The set $\mathcal{A}^{\text{mov}}_p(t)$ is defined as 
\[\mathcal{A}^{\text{mov}}_p(t) = 
\left\{ 
    a \in \mathcal{A}_p \;\middle|\; 
    \ell^+(a) \neq s_p, \quad t^-(a) < t \leq t^+(a) 
\right\}\]
where $t^-(a)$, $t^+(a)$ are respectively the departure and arrival time of arc $a$.

This optimization problem is solved using a customized local search algorithm whose neighborhood generation mechanism solely relies on perturbing the matrix $x_{imt}$. The neighborhood structure is based on 3 moves:
\begin{itemize}
    \item \textbf{Anticipating item picking.} A single item is picked earlier.
    \item \textbf{Anticipating order picking.} All items of an order are picked earlier.
    \item \textbf{Orders swaps.} Two orders assigned to the same workstation swap their position in the picking sequence.
\end{itemize}
The created neighborhood is not explored exhaustively, but a random subset of neighbors is generated at each iteration and the best improving solution is selected. This is done to speed up the optimization process: every time a new $x_{imt}$ is created, the others variable need to be update accordingly to maintain feasibility, and this can be computationally expensive especially in the reconstruction of $y_{pa}$ as feasible pod trajectories need to be defined. \\
The algorithm stops after a fixed number of iterations without improvements.
