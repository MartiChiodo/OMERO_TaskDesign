...
\texttt{ROBOT\_SPEED}                & Robot speed (cells/min, $\approx 0{,}5$ m/s) \\
...
\texttt{INTERARRIVAL\_TIME\_ORDER}   & Mean order inter-arrival time (min) \\
...
\begin{align}
    ...
    & f_{mt} \ge x_{imt},\quad g_{mt} \le x_{imt}
...
After each move, $y$ is recomputed deterministically from $x$ and $z$. Infeasible candidates are discarded; among the remaining ones, the move yielding the lowest objective is accepted. Ties are accepted to allow plateau traversal. Orders already open at a workstation are never moved. The algorithm terminates after $30$ iterations or after $5$ consecutive iterations without strict improvement.
...
\begin{itemize}[noitemsep]
    \item \textbf{Item shift.} The pick time of one item, or of a small random subset of items, is shifted earlier by a fixed number of time steps; the shift magnitude decreases when no improvement has been found in the previous iteration.
...
\eqref{M2_Constr3}--\eqref{max_active_pods} performed. If this check fails, the second-best candidate is tried before discarding the iteration. The algorithm terminates after $45$ iterations or after $3$ consecutive iterations without improvement.
